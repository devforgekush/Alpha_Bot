    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:3] == "inf":
            ...
            if await is_on_off(2):
                return await app.send_message(
                    chat_id=config.LOGGER_ID,
                    text=f"<b>🛠️ Bot Startup Notification<b>\n\n{message.from_user.mention} Just started the bot to fetch <b>Track Information<b>.\n\n<b>🆔 User ID :</b> <code>{message.from_user.id}</code>\n<b>🔗 Username :</b> @{message.from_user.username}",
                )
    else:
        out = private_panel(_)
        ...
        if await is_on_off(2):
            return await app.send_message(
                chat_id=config.LOGGER_ID,
                text=f"<b>🛠️ Bot Startup Notification<b>\n\n{message.from_user.mention} 🤖 Bot Started Successfully\n\n<b>🆔 User ID :</b> <code>{message.from_user.id}</code>\n<b>🔗 Username :</b> @{message.from_user.username}",
            )
