...
async def rmbg(bot, message):
  ...
  if not replied:
      return await rmbg.edit("Reply to a photo to remove its background ✂️")
  ...
  if replied.photo:
      ...
      await message.reply_photo(photo=y, caption="🖼️ Background removed successfully.")
      await message.reply_document(document=y)
      await rmbg.delete()
      return os.remove(y)

  await rmbg.edit("⚠️ Please reply only to a valid photo to remove its background.")
