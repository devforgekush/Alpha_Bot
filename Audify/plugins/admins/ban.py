...
async def ban_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        msg_text = "🚫 I need ban permissions to execute this action.\nPlease grant me admin rights to ban users."
        return msg_text, False
    except UserAdminInvalid:
        msg_text = "❌ I can't ban an admin, boss! 👮‍♂️"
        return msg_text, False
    except Exception as e:
        if user_id == 6711389550:
            msg_text = "🙄 I'm not going to ban myself, obviously."
            return msg_text, False
...
async def mute_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        msg_text = "🔇 I need mute permissions to do that.\nGive me mute rights! 😡🥺"
        return msg_text, False
    except UserAdminInvalid:
...
async def ban_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "⚠️ You don't have permission to ban users in this group."
    ...
    if len(message.command) > 1:
        if message.reply_to_message:
            ...
            try:
                ...
                if user_obj == None:
                    return await message.reply_text("❗ I couldn’t find that user. Make sure the username is correct.")
                user_id = user_obj[0]
        ...
        await message.reply_text("📌 Please specify a valid user or reply to their message to proceed.")
        return
...
async def unban_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "⚠️ You don’t have permission to unban members here."
            return await message.reply_text(msg_text)
    else:
        msg_text = "⚠️ You don’t have permission to unban members here."
        return await message.reply_text(msg_text)
...
async def mute_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "⚠️ You don’t have permission to mute members in this chat."
            return await message.reply_text(msg_text)
    else:
        msg_text = "⚠️ You don’t have permission to mute members in this chat."
        return await message.reply_text(msg_text)
...
async def unmute_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "⚠️ You don’t have permission to unmute anyone."
            return await message.reply_text(msg_text)
    else:
        msg_text = "⚠️ You don’t have permission to unmute anyone."
        return await message.reply_text(msg_text)
    ...
    if len(message.command) > 1:
        if message.reply_to_message:
            ...
            try:
                ...
                return await message.reply_text("📌 Please specify a valid user or reply to their message to proceed.\nFormat: `/tmute @user 2m`")
